...
**推荐使用Minimind原始数据集（默认配置）：**
- `pretrain_hq.jsonl` (1.6GB) - 预训练数据集，已预处理完成
- `sft_512.jsonl` (7.5GB) - 监督微调数据集
- `dpo.jsonl` (909MB) - DPO强化学习数据集

这些数据集包含更简单的知识，更适合小型LLM学习，训练效果更好。训练脚本已默认使用这些数据集。
...
# 步骤 1: 预训练（学知识）- 使用pretrain_hq.jsonl，10 epochs
cd trainer
python train_pretrain.py --use_wandb

# 步骤 2: 监督微调（学对话）- 使用sft_512.jsonl，2 epochs
python train_full_sft.py --use_wandb

# 步骤 3: DPO强化学习（可选，偏好对齐）- 使用dpo.jsonl
...
# 步骤 1: 预训练（学知识）- 8 GPU，使用pretrain_hq.jsonl，10 epochs
cd trainer
torchrun --nproc_per_node=8 train_pretrain.py --use_wandb

# 步骤 2: 监督微调（学对话）- 8 GPU，使用sft_512.jsonl，2 epochs
torchrun --nproc_per_node=8 train_full_sft.py --use_wandb

# 步骤 3: DPO强化学习（可选，偏好对齐）- 8 GPU，使用dpo.jsonl
...
- **默认配置**：预训练使用 `pretrain_hq.jsonl` (10 epochs)，SFT使用 `sft_512.jsonl` (2 epochs)，DPO使用 `dpo.jsonl`
- 可通过 `--data_path` 和 `--epochs` 参数自定义数据集和训练轮数
...
<details style="color:rgb(128,128,128)">
<summary>如何从chinese-fineweb等大规模数据集自行准备训练数据</summary>

如果你想从头开始准备自己的训练数据，可以使用以下脚本从大规模原始数据集（如chinese-fineweb）提取和处理数据。

**注意：** 从chinese-fineweb等大规模通用数据集提取的训练数据通常包含复杂知识，对小型LLM（如MiniMind）来说较难学习，训练效果可能不如使用Minimind原始数据集。因此，**仅建议有特殊需求或想要实验不同数据源的用户使用此方法。**

### 数据准备步骤

1. **下载原始数据集**
   ```bash
   # 从 HuggingFace 下载 chinese-fineweb 或其他数据集
   # 或从其他数据源获取原始文本数据
   ```
...
2. **准备预训练数据**
   ...
   ```
   该脚本将处理原始文本数据，清洗并格式化为预训练所需的jsonl格式。

3. **准备SFT数据**
   ```bash
   python prepare_sft_data.py
   ```
   该脚本将处理对话数据，格式化为SFT所需的格式。
...
**数据格式要求：**
- 预训练数据格式：`{"text": "你的文本内容..."}`
- SFT数据格式：`{"conversations": [{"role": "user", "content": "问题"}, {"role": "assistant", "content": "回答"}]}`
...
> 
> **推荐使用Minimind原始数据集进行训练，这些数据包含更简单的知识，更适合小型LLM学习。** 如需从chinese-fineweb等数据源自行准备数据集，请参考上方"自定义数据集准备"章节。
...
将下载的数据集文件放到`./dataset/`目录下（✨为默认推荐配置）
...
├── pretrain_hq.jsonl (1.6GB, ✨默认)
├── r1_mix_1024.jsonl (340MB)
├── sft_1024.jsonl (5.6GB)
├── sft_2048.jsonl (9GB)
├── sft_512.jsonl (7.5GB, ✨默认)
...
* `pretrain_hq.jsonl`✨ --预训练数据集，整合自jiangshu科技（**默认配置，训练脚本默认10 epochs**）
...
* `sft_512.jsonl`✨ --整合自匠数科技SFT数据，每条数据字符最大长度为512（**默认配置，训练脚本默认2 epochs**）
...
**【推荐】默认配置 - Minimind原始数据集：**
* **`pretrain_hq.jsonl` + `sft_512.jsonl` + `dpo.jsonl`** （默认，训练脚本已配置）
* 这些数据集包含更简单的知识，更适合小型LLM学习，训练效果更佳
* 预训练10 epochs，SFT 2 epochs，DPO 2 epochs
* 开销：💰💰💰，效果：😊😊😊😊

**【快速版】Zero模型快速复现：**
* 使用`pretrain_hq.jsonl` + `sft_mini_512.jsonl`最快速度复现Zero聊天模型
* 适合单卡GPU或在乎短时间快速复现的朋友
* 开销：💰，效果：😊😊
...
> **新功能！** MiniMind现在支持Muon优化器，可以显著提升训练效率！

[Muon](https://kellerjordan.github.io/posts/muon/) (MomentUm Orthogonalized by Newton-schulz) 是一种专门为神经网络隐藏层设计的优化器。
它通过Newton-Schulz迭代对2D参数（如注意力层和FFN层）的更新进行正交化处理，从Karpathy等人的NanoGPT speedrunning项目中验证可以显著提升训练速度。

**主要优势：**
- ✅ **样本效率提升35-45%**: 达到相同loss需要更少的训练步数
- ✅ **总训练时间减少25-30%**: 虽然单步略慢，但总体更快
- ✅ **训练稳定性**: 结合QK Normalization可获得更稳定的训练曲线
- ✅ **特别适合大规模预训练**: 数据集越大，效果越明显

**何时使用Muon：**
- ✅ **推荐**: 预训练阶段（17GB+ 大数据集）- 最大收益
- ⚠️ **可选**: SFT阶段（7GB数据集）- 仍有15-25%提升但配置较复杂
- ⚠️ **实验性**: DPO阶段 - 需谨慎调参
...
**默认参数（已在脚本中配置）：**
- **Pretrain**: `learning_rate_muon=0.02`, `learning_rate=5e-4`, `momentum=0.95`
- **SFT**: `learning_rate_muon=0.005`, `learning_rate=5e-7`, `momentum=0.90`
- **DPO**: `learning_rate_muon=0.001`, `learning_rate=1e-8`, `momentum=0.85`

> 💡 大多数情况下使用默认参数即可。如需调整，可通过命令行参数覆盖，例如：
> ```bash
> python train_pretrain_muon.py --learning_rate_muon 0.025 --momentum 0.93
> ```

**工作原理：**
Muon使用双优化器策略：
- **Muon优化器**: 处理2D参数（q_proj, k_proj, v_proj, o_proj, gate_proj, up_proj, down_proj）- 约占总参数的80-85%
- **AdamW优化器**: 处理1D/0D参数（embed_tokens, 所有LayerNorm层, lm_head）- 约占总参数的15-20%

**架构增强：**
MiniMind还集成了**QK Normalization**（Qwen3风格）以提高训练稳定性：
- 在应用RoPE之前对Query和Key向量进行归一化
- 防止注意力分数爆炸

> **注意**: Muon训练后的模型权重文件保存为: `pretrain_muon_*.pth`, `full_sft_muon_*.pth`, `dpo_muon_*.pth`
