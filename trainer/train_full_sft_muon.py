...
def train_epoch(epoch, wandb):
    ...
    for step, (X, Y, loss_mask) in enumerate(train_loader):
        ...
        if (step + 1) % args.save_interval == 0 and (not ddp or dist.get_rank() == 0):
            ...
            ckp = f'{args.save_dir}/full_sft_muon_{lm_config.hidden_size}{moe_path}.pth'
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="MiniMind Full SFT with Muon Optimizer")
    parser.add_argument("--out_dir", type=str, default="../out")
    parser.add_argument("--epochs", type=int, default=2)
    parser.add_argument("--batch_size", type=int, default=16)
    parser.add_argument("--learning_rate", type=float, default=5e-7, help="AdamW LR for embeddings/norms")
    parser.add_argument("--learning_rate_muon", type=float, default=0.005, help="Muon LR for SFT (lower than pretrain)")
    parser.add_argument("--momentum", type=float, default=0.90, help="Momentum for Muon (lower for SFT)")
    parser.add_argument("--device", type=str, default="cuda:0" if torch.cuda.is_available() else "cpu")
    parser.add_argument("--dtype", type=str, default="bfloat16")
    parser.add_argument("--use_wandb", action="store_true")
    parser.add_argument("--wandb_project", type=str, default="MiniMind-SFT-Muon")
    ...
    args.wandb_run_name = f"MiniMind-SFT-Muon-Epoch{args.epochs}-BS{args.batch_size}-LR_Muon{args.learning_rate_muon}"
    ...
    # Create dual optimizers with SFT-appropriate parameters
    ...
    Logger(f'⚠️  Note: Muon for SFT is experimental. Consider using standard AdamW if issues arise.')
