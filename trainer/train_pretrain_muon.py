...
def train_epoch(epoch, wandb):
    ...
    for step, (X, Y, loss_mask) in enumerate(train_loader):
        ...
        if step % args.log_interval == 0:
            ...
            Logger(
                'Epoch:[{}/{}]({}/{}) loss:{:.3f} lr_muon:{:.7f} lr_adamw:{:.7f} epoch_Time:{}min:'.format(
                    epoch + 1,
                    args.epochs,
                    step,
                    iter_per_epoch,
        ...
        if (step + 1) % args.save_interval == 0 and (not ddp or dist.get_rank() == 0):
            ...
            ckp = f'{args.save_dir}/pretrain_muon_{lm_config.hidden_size}{moe_path}.pth'
...
def init_model(lm_config):
    ...
    Logger(f'✨ Muon optimizes {100*sum(p.numel() for p in muon_params)/sum(p.numel() for p in model.parameters()):.1f}% of parameters')
    
    return model, tokenizer, muon_params, adamw_params
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="MiniMind Pretraining with Muon Optimizer")
    parser.add_argument("--out_dir", type=str, default="../out")
    parser.add_argument("--epochs", type=int, default=10)
    parser.add_argument("--batch_size", type=int, default=32)
    parser.add_argument("--learning_rate", type=float, default=5e-4, help="AdamW LR for embeddings/norms")
    parser.add_argument("--learning_rate_muon", type=float, default=0.02, help="Muon LR for 2D params (recommended: 0.01-0.03)")
    parser.add_argument("--momentum", type=float, default=0.95, help="Momentum for Muon (recommended: 0.90-0.95)")
    parser.add_argument("--device", type=str, default="cuda:0" if torch.cuda.is_available() else "cpu")
    parser.add_argument("--dtype", type=str, default="bfloat16")
    parser.add_argument("--use_wandb", action="store_true")
    parser.add_argument("--wandb_project", type=str, default="MiniMind-Pretrain-Muon")
    ...
    args.wandb_run_name = f"MiniMind-Pretrain-Muon-Epoch{args.epochs}-BS{args.batch_size}-LR_Muon{args.learning_rate_muon}-LR_AdamW{args.learning_rate}"
    ...
    model, tokenizer, muon_params, adamw_params = init_model(lm_config)
    ...
    # Create dual optimizers: Muon for 2D params, AdamW for rest
    if ddp and len(muon_params) > 0:
        # Use DistMuon for distributed training
    ...
    Logger(f'✅ QK Normalization enabled (Qwen3-style)')
